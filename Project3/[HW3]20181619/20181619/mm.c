...
static void *coalesce(void *bp) 
{
	...
	if (GET_ALLOC(HDRP(NEXT_BLKP(bp)))){
	    // case : FREE - FREE(bp) - ALLOC
	    add_size = GET_SIZE(HDRP(PREV_BLKP(bp)));
	    new_size += add_size;
	    PUT(FTRP(bp), PACK(new_size, 0));
	    PUT(HDRP(PREV_BLKP(bp)), PACK(new_size, 0));
	}
	else {
	    // case : FREE- FREE(bp) - FREE
	    add_size = GET_SIZE(HDRP(PREV_BLKP(bp)));
	    add_size += GET_SIZE(FTRP(NEXT_BLKP(bp)));
	    new_size += add_size;
	    PUT(HDRP(PREV_BLKP(bp)), PACK(new_size, 0));
